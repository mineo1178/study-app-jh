...
describe('reward correction preparation', () => {
  it('shortens 60 minutes to 20 minutes and atomically prepares the full deduction', () => {
    const change = prepareRewardCorrection({ session: session(), ledger: ledger(), profile: profile(), correction: shorten, now });
  ...
  it('invalidates a valid rewarded session down to zero rewards when assets are sufficient', () => {
    const change = prepareRewardCorrection({ session: session(), ledger: ledger(), profile: profile(), correction: { correctionId: 'invalid-1', validation: { status: 'invalid', reasonCodes: ['manual_invalid'] }, reason: 'manual_invalid' }, now });
  ...
  it('persists the session correction but performs no partial deduction when any asset is insufficient', () => {
    const change = prepareRewardCorrection({ session: session(), ledger: ledger(), profile: profile({ gold: 100, battleEnergy: 10, materials: { iron: 2 } }), correction: shorten, now });
  ...
  it('keeps a pending correction pending until the whole deduction is affordable, then clears it once', () => {
    const pending = prepareRewardCorrection({ session: session(), ledger: ledger(), profile: profile({ materials: { iron: 2 } }), correction: shorten, now });
    const insufficient = preparePendingRewardCorrectionRetry({ sessionId: 'session-1', ledger: pending.ledger, adjustment: pending.adjustment, profile: profile({ materials: { iron: 3 } }), integrity: pending.integrity, now: now + 1 });
    expect(insufficient).toEqual({ applied: false, reason: 'REWARD_PROFILE_INSUFFICIENT' });
    const resolved = preparePendingRewardCorrectionRetry({ sessionId: 'session-1', ledger: pending.ledger, adjustment: pending.adjustment, profile: profile(), integrity: pending.integrity, now: now + 2 });
    expect(resolved).toMatchObject({ applied: true, profile: { gold: 60, battleEnergy: 7, materials: { iron: 6 } }, ledger: { correctionStatus: 'applied' }, adjustment: { status: 'applied' }, integrity: { pendingSessionIds: [] } });
  });
  it('does not apply the same correction revision twice', () => {
    const first = prepareRewardCorrection({ session: session(), ledger: ledger(), profile: profile(), correction: shorten, now });
    const repeated = prepareRewardCorrection({ session: { ...session(), recordedSeconds: 1_200 }, ledger: first.ledger, profile: first.profile, correction: shorten, now: now + 1 });
  ...
  it('rejects reward increases and material-key changes, while correcting legacy validation without reward handling', () => {
    expect(() => prepareRewardCorrection({ session: session(), ledger: ledger(), profile: profile(), correction: { ...shorten, recordedSeconds: 4_000 }, now })).toThrow('CORRECTION_REWARD_INCREASE_NOT_ALLOWED');
    expect(() => prepareRewardCorrection({ session: session({ taskSnapshot: { subjectId: 's_science' } }), ledger: ledger(), profile: profile(), correction: shorten, now })).toThrow('CORRECTION_MATERIAL_CHANGE_NOT_SUPPORTED');
    const legacy = prepareRewardCorrection({ session: session({ legacySource: { taskId: 'legacy', historyId: 'h1' }, validation: { status: 'pending_review' } }), correction: { correctionId: 'legacy-invalid', validation: { status: 'invalid', reasonCodes: ['manual_invalid'] } }, now });
  ...
  it('allows a pending_review to invalid correction with no reward ledger', () => {
    const change = prepareRewardCorrection({ session: session({ validation: { status: 'pending_review', reasonCodes: ['clock_mismatch'] } }), correction: { correctionId: 'pending-invalid', validation: { status: 'invalid', reasonCodes: ['manual_invalid'] } }, now });
  ...
  it('keeps correction persistence payloads free of undefined values', () => {
    const applied = prepareRewardCorrection({ session: session(), ledger: ledger(), profile: profile(), correction: shorten, now });
    const pending = prepareRewardCorrection({ session: session(), ledger: ledger(), profile: profile({ materials: { iron: 1 } }), correction: shorten, now });
    const resolved = preparePendingRewardCorrectionRetry({ sessionId: 'session-1', ledger: pending.ledger, adjustment: pending.adjustment, profile: profile(), integrity: pending.integrity, now });
