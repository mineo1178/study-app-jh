...
import { buildLegacyMigrationCandidates } from '../../scripts/migrations/legacyMigrationCandidates.js';
...
export function buildLegacyMigrationPreflight({ tasks = [], studySessions = [], activeTimers = [] } = {}) {
  try {
    const { legacy, alreadyMigrated, candidates } = buildLegacyMigrationCandidates({ tasks, studySessions });
    const existingSourceDuplicates = duplicateValues(studySessions.map((session) => session.legacySource?.taskId ? sourceKey(session.legacySource) : null));
    ...
    const existingDocumentIds = new Set(studySessions.map((session) => session.id));
    ...
    const beforeSessions = getUnifiedStudySessions(tasks, studySessions);
    const projectedSessions = getUnifiedStudySessions(tasks, [...studySessions, ...candidates]);
    const blockers = [];
    ...
    return {
      ...
      existingStudySessionCount: studySessions.length,
      migrationCandidateCount: candidates.length,
      alreadyMigratedCount: alreadyMigrated.length,
      manualInvalidCount: [...studySessions, ...candidates].filter((session) => session.migrationReview?.decision === 'invalid').length,
      manualValidCount: [...studySessions, ...candidates].filter((session) => session.migrationReview?.decision === 'valid').length,
