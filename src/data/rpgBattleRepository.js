...
export function prepareWeeklyBossResult({ battle, profile, action, actionId, now }) { const change = preparePartyBattleAction({ battle, action: { ...action, actionId }, now }); const current = normalizePlayerProfile(profile); const base = { battle: change.battle, attackLedger: { schemaVersion: 1, type: 'attack', actionId, battleId: battle.battleId, outcome: change.battle.status, status: 'applied', appliedAt: now } }; if (change.battle.status === 'lost') return { ...base, profile: { ...current, activeBattleId: null, updatedAt: now }, defeatLedger: { schemaVersion: 1, type: 'defeat', battleId: battle.battleId, triggeringActionId: actionId, outcome: 'lost', expGranted: 0, status: 'applied', appliedAt: now } }; if (change.battle.status !== 'won') return base; const reward = battle.weeklyBossSnapshot.reward; const gacha = { ...current.gacha, ticketBalances: { ...current.gacha.ticketBalances, gold: number(current.gacha.ticketBalances.gold) + number(reward.goldTicket) }, starFragments: number(current.gacha.starFragments) + number(reward.starFragments), alchemyItems: { ...current.gacha.alchemyItems, [reward.alchemyItemId]: number(current.gacha.alchemyItems[reward.alchemyItemId]) + number(reward.alchemyItemQuantity) } }; const totalExp = current.totalExp + number(reward.expReward); const weekIds = [...current.weeklyBoss.clearedWeekIds, battle.weeklyBossSnapshot.weekId].slice(-12); return { ...base, battle: { ...change.battle, victory: { ...change.battle.victory, weeklyBoss: reward } }, profile: { ...current, totalExp, level: levelForTotalExp(totalExp), activeBattleId: null, gacha, weeklyBoss: { ...current.weeklyBoss, clearedWeekIds: [...new Set(weekIds)], totalClears: number(current.weeklyBoss.totalClears) + 1, lastClearedAt: now, lastBossId: battle.weeklyBossSnapshot.bossId }, updatedAt: now }, victoryLedger: { schemaVersion: 1, type: 'weekly_boss_clear', battleId: battle.battleId, weekId: battle.weeklyBossSnapshot.weekId, triggeringActionId: actionId, outcome: 'won', reward, status: 'applied', appliedAt: now } }; }
