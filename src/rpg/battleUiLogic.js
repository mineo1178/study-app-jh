...
export const buildBattleTurnFeedback = (result, enemyName = '敵') => { if (!result) return null; const lines = [result.skill?.name || '通常攻撃']; if (result.elementResult?.type === 'weak') lines.push('弱点を突いた！'); if (result.elementResult?.type === 'resist') lines.push('効果はいまひとつ'); lines.push(`${result.damage} DAMAGE`); if (result.outcome === 'won') lines.push('敵を倒した！'); else if (result.enemyCounter) lines.push(`${enemyName}の反撃！ ${result.enemyCounter.damage} DAMAGE`); return lines; };
